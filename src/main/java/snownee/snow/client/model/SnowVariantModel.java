...
public class SnowVariantModel extends ForwardingBakedModel {
	...
	public void emitBlockQuads(
		...
		if (SnowClientConfig.snowVariants && pos != null) {
			if (context.getModelData() == USE_SNOW_VARIANT || context.getModelData().has(SnowBlockEntity.OPTIONS) || state.hasProperty(
					DoublePlantBlock.HALF) && CoreModule.TILE_BLOCK.is(blockView.getBlockState(pos.below()))) {
				model = variantModel;
			}
		}
		if (model == null) {
