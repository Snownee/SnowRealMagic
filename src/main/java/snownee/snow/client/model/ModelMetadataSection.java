...
public record ModelMetadataSection(Identifier model, List<ResourceKey<Block>> overrideBlocks) {
	private static final Codec<ModelMetadataSection> CODEC = RecordCodecBuilder.create(instance -> instance.group(
			...
			ResourceKey.codec(Registries.BLOCK)
					...
					.orElse(List.of())
					.forGetter(ModelMetadataSection::overrideBlocks)).apply(instance, ModelMetadataSection::new));
